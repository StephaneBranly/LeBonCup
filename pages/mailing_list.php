...
<html>
    ...
    <?php
     _header(true);
        ...
        if($mailing_list!='news' && $mailing_list!='ads')
        {
        ...
        else
        {
            if($action=='subscribe')
            {
                $query = mysqli_query($connect,"SELECT mail_$mailing_list FROM `users` WHERE `iduser`='$user'");
                $resp = mysqli_fetch_array($query);
                if($resp[0]==0)
                {
                    $query2 = mysqli_query($connect,"UPDATE `users` 
